...
def analyse(stock_code):
    ...
    df[['total_revenue', 'total_profit', 'n_income']] = df[['total_revenue', 'total_profit',
                                                                          'n_income']] / 100000000
    ...
    jsonData['dates'] = df.columns.values.tolist()
...
def analyse_chart(stock_code):
    ...
    print('quarter_total_revenue')
    compute_yoy(income_df, 'q_gr_yoy')
    print('quarter_total_profit')
    compute_yoy(income_df, 'q_op_yoy')
    print('quarter_n_income')
    compute_yoy(income_df, 'q_profit_yoy')
    ...
    df = df['总收入季度增长率':'净利润季度增长率']
    jsonData['dates'] = df.columns.values.tolist()[1:-1]
    jsonData['data'] = []
